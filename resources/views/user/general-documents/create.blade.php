...
@section('content')
    ...
    <div class="row">
        <div class="col-xl-6">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('users.store-general-documents') }}" class="custom-validation"
                        ...
                        <div class="mb-3">
                            <label class="form-label">{{ ucwords(str_replace('_', ' ', 'General Document')) }}</label>
                            <input type="file" id="file_path" class="dropify" name="file_path" data-height="200" required>
                            @error('file_path')
                                <span class="text-red">{{ $message }}</span>
                            @enderror
                        </div>
