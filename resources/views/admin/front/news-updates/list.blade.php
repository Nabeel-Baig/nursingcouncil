...
@section('script-bottom')
    <script>
        $(function() {
            var count = 0;
            var source = `{{ route('admin.news-updates.index') }}`;
            ...
            $(document, this).on('click', '.view', function() {
                let id = $(this).attr('id');
                let url = '{{ route('admin.news-updates.show', ':id') }}';
                $.ajax({
            ...
            $(document).on('click', '#ok_delete', function() {
                let url = '{{ route('admin.news-updates.destroy', ':id') }}'
                $.ajax({
                    type: "delete",
            ...
            document.getElementById('select_all').addEventListener('click', event =>
                (event.target.checked === true) ? Array.from(document.querySelectorAll('.delete_checkbox'))
                .forEach(checkbox =>
